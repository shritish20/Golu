...
# === DYNAMIC FULL PATCH START ===

# ---------------- DYNAMIC INSTRUMENT RESOLUTION UTILS ----------------

async def get_current_and_all_expiries(access_token: str, symbol: str = "NIFTY") -> dict:
    config = await get_config(access_token)
    async with httpx.AsyncClient() as client:
        r = await client.get(
            f"{config['base_url']}/option/contract",
            headers=config['headers'],
            params={"instrument_key": f"NSE_INDEX|{symbol}"}
        )
        r.raise_for_status()
        data = r.json()["data"]
        expiries = sorted(list(set([x["expiry"] for x in data])))
        return {"current_expiry": expiries[0], "all_expiries": expiries, "raw": data}


async def resolve_instrument_key(access_token: str, symbol: str, expiry: str, strike: float, option_type: str) -> str:
    contracts = await get_current_and_all_expiries(access_token, symbol)
    for item in contracts["raw"]:
        if (item["expiry"] == expiry and
            item["strike"] == strike and
            item["instrument_type"] == option_type):
            return item["instrument_key"]
    raise ValueError(f"Instrument key not found for {symbol} {expiry} {strike} {option_type}")


async def fetch_ltp(access_token: str, instrument_key: str) -> float:
    config = await get_config(access_token)
    async with httpx.AsyncClient() as client:
        res = await client.get(
            f"{config['v3_url']}/market-quote/ltp",
            headers=config['headers'],
            params={"instrument_key": instrument_key}
        )
        res.raise_for_status()
        return res.json()["data"][instrument_key]["ltp"]
...
async def place_order(
    ...
    action: str = Query(...),       # "BUY" or "SELL"
    quantity: int = Query(50),
    order_type: str = Query("MARKET"),  # "MARKET" or "LIMIT"
    product: str = Query("NRML"),
    trigger_price: Optional[float] = Query(None)  # For SL orders
):
    try:
        expiry_data = await get_current_and_all_expiries(access_token, symbol)
        expiry = expiry or expiry_data["current_expiry"]
        instrument_key = await resolve_instrument_key(access_token, symbol, expiry, strike, option_type)
        ltp = await fetch_ltp(access_token, instrument_key)
        ...
        order_payload = {
            ...
            "product": product,
            "validity": "DAY",
            "price": ltp if order_type == "LIMIT" else 0
        }
        ...
        async with httpx.AsyncClient() as client:
            res = await client.post(f"{config['base_url']}/order/place", headers=config['headers'], json=order_payload)
            res.raise_for_status()
            return {"message": "Order placed successfully", "details": res.json()}
    except Exception as e:
...
async def place_gtt_order(
    ...
    try:
        ...
        async with httpx.AsyncClient() as client:
            res = await client.post(f"{config['base_url']}/gtt/create", headers=config['headers'], json=gtt_payload)
            res.raise_for_status()
            return {"message": "GTT order created successfully", "details": res.json()}
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
async def execute_strategy(
    access_token: str = Query(...),
    strategy_name: str = Query(...),  # e.g., "iron_fly"
    expiry: str = Query(None),
    quantity: int = Query(50),
    strike_offset: float = Query(0),
    custom_strikes: Optional[str] = Query(None)
):
    try:
        # Replace with actual Nifty spot fetch if available
        spot = 22500
        atm = round(spot / 50) * 50
        expiry_data = await get_current_and_all_expiries(access_token, "NIFTY")
        expiry = expiry or expiry_data["current_expiry"]
        ...
        if strategy_name == "iron_fly":
            ce_sell = atm + strike_offset
            pe_sell = atm - strike_offset
            ce_buy = ce_sell + 100
            pe_buy = pe_sell - 100

            legs = [
                {"strike": ce_sell, "option_type": "CE", "action": "SELL"},
                {"strike": ce_buy, "option_type": "CE", "action": "BUY"},
                {"strike": pe_sell, "option_type": "PE", "action": "SELL"},
                {"strike": pe_buy, "option_type": "PE", "action": "BUY"},
            ]

        if custom_strikes:
            legs = []
            for part in custom_strikes.split(";"):
                strike, opt_type, action = part.split("-")
                legs.append({"strike": float(strike), "option_type": opt_type, "action": action})

        responses = []
        async with httpx.AsyncClient() as client:
            for leg in legs:
                instrument_key = await resolve_instrument_key(access_token, "NIFTY", expiry, leg["strike"], leg["option_type"])
                ltp = await fetch_ltp(access_token, instrument_key)
                ...
                res = await client.post(f"{config['base_url']}/order/place", headers=config['headers'], json=payload)
                res.raise_for_status()
                responses.append(res.json())

        return {"status": "executed", "legs": responses}
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))

# === DYNAMIC FULL PATCH END ===
