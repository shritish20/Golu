    ...
    try:
        ...
        if trades_df.empty:
            ...
            for _, row in trades_df.iterrows():
                ...
                risk_ok = "âœ…" if potential_risk <= max_risk_per_trade else "âŒ"
                ...
                if risk_ok == "âŒ":
                    flags.append(f"âŒ {strat} exceeded its per-trade risk limit (Potential Risk: {potential_risk:.2f}, Limit: {max_risk_per_trade:.2f})")
                if sl_hit:
        ...
        if total_risk_on_table > daily_risk_limit:
            flags.append(f"âŒ Total portfolio risk ({total_risk_on_table:.2f}) exceeds daily limit ({daily_risk_limit:.2f}).")

        if net_dd > max_drawdown:
            flags.append(f"âŒ Portfolio drawdown ({net_dd:.2f}) exceeds maximum allowed ({max_drawdown:.2f}).")
