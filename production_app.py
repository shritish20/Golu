...
# --- SUPABASE CLIENT ---
def log_trade_to_supabase(data: dict):
    try:
        data["timestamp_entry"] = datetime.utcnow().isoformat()
        data["timestamp_exit"] = datetime.utcnow().isoformat()
        data["status"] = "closed"
        response = requests.post(f"{SUPABASE_URL}/rest/v1/trade_logs", json=data, headers=HEADERS)
        return response.status_code, response.json()
    except Exception as e:
        logger.error(f"Error logging trade to Supabase: {e}")
        return 500, {"error": str(e)}

def add_journal_to_supabase(data: dict):
    try:
        data["timestamp"] = datetime.utcnow().isoformat()
        response = requests.post(f"{SUPABASE_URL}/rest/v1/journals", json=data, headers=HEADERS)
        return response.status_code, response.json()
    except Exception as e:
        logger.error(f"Error adding journal to Supabase: {e}")
        return 500, {"error": str(e)}

# --- FastAPI Setup ---
app = FastAPI(
    title="VoluGuard API", 
    description="Production-ready options trading strategy API",
    version="1.0.0"
)
...
def log_trade(trade_data):
    try:
        status, result = log_trade_to_supabase(trade_data)
        if status == 201:
            return True
        else:
            logger.warning(f"Error logging trade to Supabase: {result}")
            return False
    except Exception as e:
...
def add_journal_entry(entry_data):
    try:
        status, result = add_journal_to_supabase(entry_data)
        if status == 201:
            return True
        else:
            logger.warning(f"Error adding journal entry to Supabase: {result}")
            return False
    except Exception as e:
        logger.error(f"Exception in add_journal_entry: {e}")
        return False

def get_all_trades():
    try:
        response = requests.get(f"{SUPABASE_URL}/rest/v1/trade_logs", headers=HEADERS)
        if response.status_code == 200:
            return response.json()
        else:
            logger.warning(f"Error fetching trades: {response.text}")
            return []
    except Exception as e:
        logger.error(f"Exception in get_all_trades: {e}")
        return []

def get_all_journals():
    try:
        response = requests.get(f"{SUPABASE_URL}/rest/v1/journals", headers=HEADERS)
        if response.status_code == 200:
            return response.json()
        else:
            logger.warning(f"Error fetching journals: {response.text}")
            return []
    except Exception as e:
        logger.error(f"Exception in get_all_journals: {e}")
        return []
...
def get_config(access_token):
    ...
    def get_next_expiry_internal():
        try:
            ...
            logger.warning(f"Error fetching expiries: {res.status_code} - {res.text}")
            return datetime.now().strftime("%Y-%m-%d")
        except Exception as e:
            logger.error(f"Exception in get_next_expiry: {e}")
            return datetime.now().strftime("%Y-%m-%d")
...
def fetch_option_chain(config):
    try:
        ...
        logger.warning(f"Error fetching option chain: {res.status_code} - {res.text}")
        return []
    except Exception as e:
...
def extract_seller_metrics(option_chain, spot_price):
    try:
        ...
        logger.error(f"Exception in extract_seller_metrics: {e}")
        return {}
...
def load_xgboost_model():
    try:
        ...
        if response.status_code == 200:
            ...
            return model
        logger.warning(f"Error fetching XGBoost model: {response.status_code} - {response.text}")
        return None
    except Exception as e:
...
async def root(request: Request):
    return JSONResponse(content={"message": "VoluGuard API is live ✅", "version": "1.0.0"})

@app.get("/predict/volatility")
def predict_volatility(access_token: str):
    try:
...
@app.post("/log/trade")
def log_new_trade(trade: TradeRequest):
    try:
        success = log_trade(trade.dict())
        if not success:
            raise HTTPException(status_code=500, detail="Failed to log trade")
        return {"status": "success"}
    except Exception as e:
        logger.error(f"Error in log_new_trade: {e}")
        raise HTTPException(status_code=500, detail=str(e))

@app.post("/log/journal")
def log_new_journal(journal: JournalRequest):
    try:
        success = add_journal_entry(journal.dict())
        if not success:
            raise HTTPException(status_code=500, detail="Failed to save journal")
        return {"status": "success"}
    except Exception as e:
        logger.error(f"Error in log_new_journal: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
def fetch_journals():
    try:
        return {"journals": get_all_journals()}
    except Exception as e:
        logger.error(f"Error in fetch_journals: {e}")
        raise HTTPException(status_code=500, detail=str(e))

@app.get("/fetch/option-chain")
def fetch_option_chain_endpoint(access_token: str):
    try:
        config = get_config(access_token)
        data = fetch_option_chain(config)
        return {"data": data}
    except Exception as e:
        logger.error(f"Error in fetch_option_chain_endpoint: {e}")
        raise HTTPException(status_code=500, detail=str(e))

@app.post("/order/place-multi-leg")
def place_multi_leg_orders_endpoint(orders: List[Dict], access_token: str):
    try:
        config = get_config(access_token)
        # Placeholder for actual order placement logic
        return {"status": "Order received", "orders": orders}
    except Exception as e:
        logger.error(f"Error in place_multi_leg_orders_endpoint: {e}")
        raise HTTPException(status_code=500, detail=str(e))

@app.get("/suggest/strategy")
def suggest_strategy_endpoint(access_token: str):
    try:
        config = get_config(access_token)
        option_chain = fetch_option_chain(config)
        if not option_chain:
        ...
        ivp = 80
        vix = 20
        iv_rv_spread = round(seller["avg_iv"] - 18, 2)
        iv_skew_slope = 0.0005
        ...
        logger.error(f"Error in suggest_strategy_endpoint: {e}")
        raise HTTPException(status_code=500, detail=str(e))

@app.post("/strategy/details")
def get_strategy_details_endpoint(req: StrategyRequest, access_token: str):
    try:
        config = get_config(access_token)
        option_chain = fetch_option_chain(config)
        if not option_chain:
        ...
        logger.error(f"Error in get_strategy_details_endpoint: {e}")
        raise HTTPException(status_code=500, detail=str(e))

@app.get("/risk/portfolio")
def evaluate_risk(access_token: str):
    try:
        config = get_config(access_token)
        option_chain = fetch_option_chain(config)
        if not option_chain:
...
def full_chain_table_endpoint(access_token: str):
    try:
        config = get_config(access_token)
        option_chain = fetch_option_chain(config)
        if not option_chain:
        ...
        logger.error(f"Error in full_chain_table_endpoint: {e}")
        raise HTTPException(status_code=500, detail=str(e))

@app.get("/calculate/regime")
def calculate_regime_endpoint(access_token: str):
    try:
        config = get_config(access_token)
        option_chain = fetch_option_chain(config)
        if not option_chain:
        ...
        logger.error(f"Error in calculate_regime_endpoint: {e}")
        raise HTTPException(status_code=500, detail=str(e))

@app.get("/calculate/iv-skew-slope")
def calculate_iv_skew_slope_endpoint(access_token: str):
    try:
        config = get_config(access_token)
        option_chain = fetch_option_chain(config)
        if not option_chain:
        ...
        logger.error(f"Error in calculate_iv_skew_slope_endpoint: {e}")
        raise HTTPException(status_code=500, detail=str(e))

@app.get("/calculate/chain-metrics")
def chain_metrics_endpoint(access_token: str):
    try:
        config = get_config(access_token)
        option_chain = fetch_option_chain(config)
        if not option_chain:
        ...
        logger.error(f"Error in chain_metrics_endpoint: {e}")
        raise HTTPException(status_code=500, detail=str(e))

@app.get("/calculate/volatility")
def calc_volatility_endpoint(access_token: str):
    try:
        config = get_config(access_token)
        option_chain = fetch_option_chain(config)
        if not option_chain:
        ...
        logger.error(f"Error in calc_volatility_endpoint: {e}")
        raise HTTPException(status_code=500, detail=str(e))

@app.get("/load/model")
def load_xgboost_model_endpoint():
    try:
        ...
        logger.error(f"Error in load_xgboost_model_endpoint: {e}")
        raise HTTPException(status_code=500, detail=str(e))

@app.get("/predict/xgboost-vol")
def predict_xgboost_vol_endpoint(access_token: str):
    try:
        config = get_config(access_token)
        option_chain = fetch_option_chain(config)
        if not option_chain:
        ...
        logger.error(f"Error in predict_xgboost_vol_endpoint: {e}")
        raise HTTPException(status_code=500, detail=str(e))

@app.get("/calculate/risk")
def calculate_risk_endpoint(access_token: str):
    try:
        config = get_config(access_token)
        option_chain = fetch_option_chain(config)
        if not option_chain:
        ...
        logger.error(f"Error in calculate_risk_endpoint: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
def test_all_endpoints(access_token: str):
    try:
        ...
        regime = calculate_regime_endpoint(access_token)
        iv_skew = calculate_iv_skew_slope_endpoint(access_token)
        vol = calc_volatility_endpoint(access_token)
        xgb_vol = predict_xgboost_vol_endpoint(access_token)
        risk = calculate_risk_endpoint(access_token)
        strategies = suggest_strategy_endpoint(access_token)
        ...
        logger.error(f"Error in test_all_endpoints: {e}")
        raise HTTPException(status_code=500, detail=str(e))

@app.get("/ws/market-url")
def get_market_websocket_url(access_token: str):
    """
    ...
    try:
        ...
        if response.status_code == 200:
            data = response.json()
            return {"ws_url": data["data"]["authorized_redirect_uri"]}
        else:
            raise HTTPException(status_code=response.status_code, detail=response.text)
    except Exception as e:
        logger.error(f"Error in get_market_websocket_url: {e}")
        raise HTTPException(status_code=500, detail=str(e))

@app.get("/ws/portfolio-url")
def get_portfolio_websocket_url(access_token: str):
    """
    ...
    try:
        ...
        logger.error(f"Error in get_portfolio_websocket_url: {e}")
        raise HTTPException(status_code=500, detail=str(e))
